						...
						<c:forEach items="${articleListVoList}" var="a">
							...
							<article class="post">
								...
								<figure class="thumbnail">
									...
									<span class="cat">
										<a href="/category/${a.categoryCustomList[fn:length(a.categoryList)-1].categoryId}">
											${a.categoryCustomList[fn:length(a.categoryList)-1].categoryName} </a>
									</span>
								</figure>
				...
				<c:if test="${articleListVoList.size()!=0}">
					...
					<nav class="navigation pagination" role="navigation">
						<div class="nav-links">
							...
							<%--显示第一页的页码--%>
							<c:if test="${begin >= 2 }">
								<a class="page-numbers" href="/category/${articleListVoList[0].categoryCustomList[0].categoryId}/p/1">1</a>
							</c:if>
							<%--显示点点点--%>
							<c:if test="${begin  > 2 }">
							...
							<c:forEach begin="${begin }" end="${end }" var="i">
								<c:choose>
									...
									<c:otherwise>
										<a class="page-numbers" href="/category/${articleListVoList[0].categoryCustomList[0].categoryId}/p/${i}">${i }</a>
									</c:otherwise>
								</c:choose>
							</c:forEach>
							<%-- 显示点点点 --%>
